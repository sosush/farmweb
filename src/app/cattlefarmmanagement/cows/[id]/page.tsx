// src/app/cattlefarmmanagement/cows/[id]/page.tsx
'use client'; // This directive is crucial for client-side hooks like useState, useEffect, useParams, useRouter

import { useEffect, useState } from 'react';
import { useParams, useRouter } from 'next/navigation'; // Correct imports for App Router
import { format } from 'date-fns';
import { Edit, Trash2, Plus } from 'lucide-react';
import { CowService } from '../../../../cattlefarmmanagement/lib/cowService'; // Adjusted import path
import type { Cow, HealthRecord, BreedingRecord } from '../../../../cattlefarmmanagement/types'; // Adjusted import path
import AddEditCowModal from '../../../../cattlefarmmanagement/components/cows/AddEditCowModal'; // Adjusted import path
import AddRecordModal from '../../../../cattlefarmmanagement/components/cows/AddRecordModal'; // Adjusted import path

export default function CowDetailsPage() { // Renamed the function to reflect its page role
  ...
  const loadCowData = async () => {
    try {
      setLoading(true);
      setError('');
      // Assuming CowService can fetch all cows and then find the specific one by ID
  ...
  const handleSaveRecord = async (record: Omit<HealthRecord | BreedingRecord, 'id'>) => {
    try {
      if (addRecordModal === 'health') {
        // Assuming your CowService.saveHealthRecord expects a complete HealthRecord object
        // and that cow_id is a required field for new records.
        await CowService.saveHealthRecord({ ...record, cow_id: id } as Omit<HealthRecord, 'id'>);
      } else {
        // Assuming your CowService.saveBreedingRecord expects a complete BreedingRecord object
      ...
      // Re-throw the error so the calling component (modal) can catch it if needed
      throw error;
    }
  };
  ...
  const handleDelete = async () => {
    try {
      ...
      router.push('/cattlefarmmanagement/cows'); // Correct App Router navigation
    } catch (error) {
    ...
    <div className="space-y-6">
      ...
      {addRecordModal && (
        <AddRecordModal
          ...
          cowId={id} // Ensure cowId is passed
